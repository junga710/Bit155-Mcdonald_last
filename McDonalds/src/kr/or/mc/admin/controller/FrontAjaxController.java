...
public class FrontAjaxController extends HttpServlet {
    ...
    private void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (url_Command.equals("/CategoryList.ajax")) { // 회원등록 페이지 카테고리
			action = new CategoryListService();
			forward = action.execute(request, response);
		} else if(url_Command.equals("/MainSingleFour.ajax")) { //메인 페이지 단일값 위쪽 4개
			action = new MainSingleFourService();
			forward = action.execute(request, response);
		} else if(url_Command.equals("/MainChart.ajax")) { //메인 페이지 단일값 위쪽 4개
